...
export default async function ContestPage() {
  ...
  const [contests, topUsers] = await Promise.all([
    getCachedContestList().then((contests) => contests.map((c) => ({
      id: c.id,
      slug: c.slug,
      title: c.title,
  ...
  const [registeredContestIds, myStats] = !userId ? [[], null] :
    await Promise.all([
      ...
      prisma.user.findUnique({
        ...
        rating: user.rating,
        streak: user.streak,
        solvedTotal: user.solvedEasy + user.solvedMedium + user.solvedHard,
